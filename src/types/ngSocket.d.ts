/**
 * Интерфейс запроса, получаемого из NgSocket при входящем сообщении. Используется для отправки ответа на это входящее сообщение
 */
...
export interface IResultDescription {
  /**
   * Код результата.
   * Общее соглашение по возвращаемым кодам:
   * >= 0 - успешно
   * < 0 - ошибка.
   * -1.. - 1000 - прикладные ошибки, описание смотри в соответствующем запросе.
...
export interface INgSocket {
   ...
   * Функция вызывается, когда соответствующее представление станет активно
   * @param callBack - Функция, обработчик события
   */
  onActivate(callBack: (args: any) => void): any;

  /**
   * Добавить функцию-обработчик на получение события деактивации.
   * Функция вызывается, когда соответствующее представление закончит работу.
   * По этому событию необходимо удалить какие-либо критичные данные связанные с текущей
   * клиентской сессией
   * @param callBack - Функция, обработчик события
   */
  onDeactivate(callBack: () => void): any;
   ...
   * @param callBack - Функция, обработчик события
   */
  onError(callBack: (err: INgSocketError) => void): any;
   ...
   */
  on(name: string, callBack: ((v: any, request?: IRequest) => void) | null | undefined): any;
   ...
   */
  addEventListener(name: string, func: (v: any) => void): any;
   ...
   */
  removeEventListener(name: string, func: (v: any) => void): any;
   ...
   */
  send(name, obj?: any, okFunc?: (v: any) => void, errFunc?: (v: IResultDescription) => void): any;
   ...
   * @param name - имя функции
   * @param obj - входные данные запроса
   * @exception e - Исключение типа IResultDescription в случае ошибки
   */
...
export interface GlobalNgSocket {
  ...
  /**
   * Указать адрес веб-сокета приложения
   */
